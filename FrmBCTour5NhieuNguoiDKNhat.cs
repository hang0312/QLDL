...
namespace QuanLyCongTyDuLich
{
    public partial class FrmBC3 : Form
    {
        ...
        {
            string sql = "SELECT TOP 5 d.MaTour, d.TenTour,d.MaCongTy,c.SoNguoiDKTour " +
                         "FROM DanhMucTour as d inner join(SELECT a.MaTour, SUM(b.b1) as SoNguoiDKTour " +
                                                                "FROM LichTour as a inner join(SELECT MaLichTour, SUM(SoLuongDangKy) as b1 " +
                                                                                                    "FROM DangKiTour " +
                                                                                                     "GROUP BY MaLichTour) as b " +
                                                                "ON a.MaLichTour = b.MaLichTour " +
                                                                "Group by a.MaTour) as c " +
                        "ON d.MaTour = c.MaTour "+
                        "ORDER BY c.SoNguoiDKTour desc";
            ...
            dataGridViewBC3.Columns[3].HeaderText = "Số người ĐK Tour";
        ...
        private void btnInBC3_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["C2:G2"].Value = "Báo cáo 5 tour có nhiều người ĐK nhất";
            sql = "SELECT TOP 5 d.MaTour, d.TenTour,d.MaCongTy,c.SoNguoiDangKyTour " +
                         "FROM DanhMucTour as d inner join(SELECT a.MaTour, SUM(b.b1) as SoNguoiDangKyTour " +
                                                                "FROM LichTour as a inner join(SELECT MaLichTour, SUM(SoLuongDangKy) as b1 " +
                                                                                                    "FROM DangKiTour " +
                                                                                                     "GROUP BY MaLichTour) as b " +
                                                                "ON a.MaLichTour = b.MaLichTour " +
                                                                "Group by a.MaTour) as c " +
                        "ON d.MaTour = c.MaTour " +
                        "ORDER BY c.SoNguoiDangKyTour desc";
            ...
            exRange.Range["F5:F5"].Value = "Số người Đk Tour";
